...
import base64
import subprocess  # 🔄 Required to execute update_tokens.py
import sys         # 🔄 Required to find python executable path
...
def index():
    return jsonify({
        "credit": "https://t.me/THEEAGLE87DR",
        "message": "Welcome to the BORZ Like API",
...
# 🛰️ === NEW SECURE TOKEN TRIGGER ENDPOINT ===
@app.route('/refresh_tokens_secure_endpoint', methods=['GET', 'POST'])
def refresh_tokens_api():
    try:
        app.logger.info("⚡ Token refresh sequence triggered from Telegram Bot.")
        # Trigger 'update_tokens.py' on Vercel environment locally
        result = subprocess.run([sys.executable, "update_tokens.py"], capture_output=True, text=True)
        
        # Checking if execution has error codes
        if result.returncode != 0:
            app.logger.error(f"Error executing update_tokens.py: {result.stderr}")
            return jsonify({
                "status": "error",
                "message": "Script executed but failed to refresh.",
                "details": result.stderr[:200]
            }), 500
            
        return jsonify({
            "status": "success",
            "message": "Tokens updated successfully on Vercel server!",
            "output": result.stdout[:200]
        }), 200
    except Exception as e:
        app.logger.error(f"Exception in trigger endpoint: {e}")
